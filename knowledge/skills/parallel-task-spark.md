# Canonical Skill: parallel-task-spark

Generated at: `2026-03-18T19:24:23+00:00`
Translation status: `english`
Canonical ID: `skill--parallel-task-spark`

## Summary
>

## Provenance
- `am-will/codex-skills` `main` `skills/parallel-task-spark/SKILL.md` ([raw](https://raw.githubusercontent.com/am-will/codex-skills/main/skills/parallel-task-spark/SKILL.md))

## Metadata
- Tags: `parallel-task-spark, skill, skills`
- Languages: ``
- Systems: ``
- Jobs: `parallel-task-spark, skills`
- Roles: `orchestrator`

## Steps
1. **Plan file**: The markdown plan to read
2. **Task subset** (optional): Specific task IDs to run
3. **agent_type**: `sparky` (Sparky role)
4. **depends_on** list (from `- **depends_on**: [...]`)
5. **description**: "Implement task [ID]: [name]"
6. **prompt**: Use template below
7. Build task list
8. Constraints: [risks from plan]
9. Dependencies: [prerequisites for this task]
10. Find task subsections (e.g., `### T1:` or `### Task 1.1:`)
11. For each task, extract:
12. Full content (description, location, acceptance criteria, validation)
13. Goals: [relevant overview from plan]
14. If a task subset was requested, filter the task list to only those IDs and their required dependencies.
15. Plan: [filename]
16. Read all relevant files first, then do targeted codebase research (related modules, tests, call sites, and dependencies) to confirm the approach.
17. Read the working plan and fully understand this task before coding.
18. Related tasks: [tasks that depend on or are depended on by this task]
19. Task ID and name
20. Use the `sparky` agent role for this task; do not use any other role.

## Instructions
Source: am-will/codex-skills (main) :: skills/parallel-task-spark/SKILL.md
